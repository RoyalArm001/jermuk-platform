...
import { translatePage } from "./lib/translateClient";
...
export function setLang(code: "hy" | "ru" | "en") {
  i18n.changeLanguage(code).then(() => {
    // Translate only explicitly marked content nodes (data-tr / data-i18n)
    // UI strings still use react-i18next resources above.
    try {
      translatePage(code);
    } catch {}
  });
